...
public class BuildMapperXml {
    ...
    public static void execute(TableInfo tableInfo) {
        File folder = new File(Constans.PATH_MAPPERS_XML);
        if (!folder.exists()) {
            folder.mkdirs();
        }
        ...
        try {
            ...
            bw.newLine();
            bw.write("<mapper namespace=\"" + Constans.PACKAGE_MAPPERS + "." + className + "\">");
            bw.newLine();
            bw.newLine();
            bw.write("\t<!--实体映射-->");
            bw.newLine();
            ...
            for (FieldInfo fieldInfo : tableInfo.getFieldList()) {
                String stringQuery = "";
                if (ArrayUtils.contains(Constans.SQL_STRING_TYPES, fieldInfo.getSqlType())) {
            ...
            for (FieldInfo fieldInfo : tableInfo.getFieldExtendList()) {
                ...
                } else if (ArrayUtils.contains(Constans.SQL_DATE_TIME_TYPES, fieldInfo.getSqlType()) || ArrayUtils.contains(Constans.SQL_DATE_TYPES, fieldInfo.getSqlType())) {
            ...
            for (FieldInfo fieldInfo : tableInfo.getFieldList()) {
                if (fieldInfo.getAutoIncrement() != false || fieldInfo.getAutoIncrement()) {
            ...
            //批量插入或更新
            bw.newLine();
            bw.write("\t<!--添加（批量插入或更新）||(需要将之前存在的数据记录，否则会覆盖，例如假如原有code=1，更新时没有设置code=1，只设置了createtime，那么code就会为null)-->");
            bw.newLine();
            ...
            bw.write("\t<!--多条件修改-->");
            bw.newLine();
            bw.write("\t<update id=\"updateByQuery\" parameterType=\"" + Constans.PACKAGE_QUERY + "." + tableInfo.getBeanName() + "Query\">");
            bw.newLine();
            ...
            bw.write("\t<!--多条件删除-->");
            bw.newLine();
            bw.write("\t<delete id=\"deleteByQuery\">");
            bw.newLine();
            ...
            logger.error("创建Mappers XML文件失败", e);
        } finally {
            if (bw != null) {
                try {
                    bw.close();
